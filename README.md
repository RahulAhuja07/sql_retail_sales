...
-- **How many unique customer we have  we have?**
SELECT COUNT(DISTINCT customer_id) as total_sale FROM retail_sales;


-- **How many unique category we have  we have?**
...
-- Deleting if there is NULL values(deleting the full row)
...
SET SQL_SAFE_UPDATES = 1;  -- again setting the safe updates to 1.
```
...
SELECT 
    IF(HOUR(sale_time) < 12, 'Morning',
       IF(HOUR(sale_time) <= 17, 'Afternoon', 'Evening')
    ) AS shift,
    COUNT(*) AS total_orders
FROM retail_sales
GROUP BY shift;
...
2. Load the csv file to the database and run the desired queries.
